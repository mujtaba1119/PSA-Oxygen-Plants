...
import RemoteMonitoring from "./RemoteMonitoring.jsx";
...
const fullHospitalName = h => { const n = FULL_HOSPITAL_NAMES[h] || h; if (!n.includes("DHQ")) return n; return n.replace("DHQ Teaching Hospital", "THQ").replace("DHQ Hospital", "DHQ").replace(" - DHQ", " - DHQ"); };
...
// Update an existing shutdown record (admin manual edit).
async function updateShutdownRecord(id, fields) {
  const { error } = await supabase.from("shutdowns").update(fields).eq("id", id);
  if (error) console.error(error);
  return !error;
}
// Delete a shutdown record.
async function deleteShutdownRecord(id) {
  const { error } = await supabase.from("shutdowns").delete().eq("id", id);
  if (error) console.error(error);
  return !error;
}
// Insert a shutdown record manually (admin) with explicit dates.
async function insertShutdownRecord(hospital, startDate, endDate, reason, note, loggedBy) {
  const { error } = await supabase.from("shutdowns").insert({ hospital, start_date: startDate, end_date: endDate || null, reason, note: note || null, logged_by: loggedBy || null });
  if (error) console.error(error);
  return !error;
}
  ...
  return (
    <div className="header-reveal top-bar-responsive" style={styles.topBar}>
      ...
      <div className="top-center-responsive" style={styles.topCenter}>
        {LOGO_GLOBALFUND && <img className="gf-logo" src={LOGO_GLOBALFUND} alt="Global Fund" style={{ height: 64, objectFit: "contain" }} />}
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 16px", background: "linear-gradient(180deg, #062825 0%, #0b3b38 30%, #0f5650 55%, #0f766e 80%, #14a89a 100%)", fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif' }}>
      ...
      <div className="login-logo-panel" style={{ width: "100%", maxWidth: 640, marginTop: 26, background: "#fff", borderRadius: 18, overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.2)" }}>
        ...
        <div className="login-logo-row" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "24px 20px", gap: 6 }}>
          <img className="lg-img lg-gf" src={LOGO_GLOBALFUND} alt="Global Fund" style={{ height: 52, objectFit: "contain" }} />
...
function HomeTab({ hospitals, groups, complaints, siteNotes, shutdowns = [], onViewSite, user }) {
  ...
  // The UNDP/CMU oversight dashboard is also used by admin and all Amex users.
  const isUndpCmu = user && (
    user.role === "admin" ||
    (user.role === "company" && ["UNDP", "CMU", "Amex"].includes(getCompanyName(user)))
  );
  ...
  // Downtime / uptime — equipment-fault shutdowns only. Uptime measured over the network's
  // full operational lifetime (earliest recorded activity → today), per site.
  ...
  // Determine how long the network has been operating: earliest complaint or shutdown date.
          ...
          <button key={i} onClick={() => viewFile(file)} disabled={loading} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 12.5, fontWeight: 500, color: "#1a2332", background: "#f8fffe", padding: "8px 12px", borderRadius: 10, border: `1px solid ${C.tealLight}`, cursor: loading ? "wait" : "pointer", textAlign: "left", width: "fit-content", maxWidth: "100%", transition: "background 0.15s" }} onMouseEnter={e => { if (!loading) e.currentTarget.style.background = "#e6f5f0"; }} onMouseLeave={e => { e.currentTarget.style.background = "#f8fffe"; }}>
            <div style={{ width: 30, height: 30, borderRadius: 8, background: isImage ? "#e6f5f0" : isPdf ? "#fef2f2" : "#f0f4ff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              {isImage
                ...
                : isPdf
                  ? <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
...
function EquipmentTab({ hospitals, complaints, siteNotes, shutdowns = [], isAdmin, onRefresh }) {
  ...
  const [editingSd, setEditingSd] = useState(null); // shutdown record id being edited
    ...
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div className="main-area" style={{ flex: 1, marginLeft: 180, background: "#f7f8fa", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        <main style={{ maxWidth: 1060, margin: "0 auto", padding: "28px 32px", width: "100%", flex: 1 }}>
          ...
          {tab === "equipment" && <EquipmentTab hospitals={ALL_HOSPITALS} complaints={complaints} siteNotes={siteNotes} shutdowns={shutdowns} isAdmin={true} onRefresh={onRefresh} />}
    ...
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div className="main-area" style={{ flex: 1, marginLeft: 180, background: "#f7f8fa", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        <main style={{ maxWidth: 1060, margin: "0 auto", padding: "28px 32px", width: "100%", flex: 1 }}>
          ...
          {tab === "equipment" && <EquipmentTab hospitals={myHospitals} complaints={complaints} siteNotes={siteNotes} shutdowns={shutdowns} isAdmin={false} onRefresh={onRefresh} />}
          {tab === "tickets" && !selected && (<>
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
